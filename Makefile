...
PROJECT_NAME=pssa2023
STACK_NAME=pssa2023-ericof-tangrama-com-br
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://pssa2023.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t ericof/pssa2023-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t ericof/pssa2023-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name pssa2023-backend-acceptance -d ericof/pssa2023-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name pssa2023-frontend-acceptance --link pssa2023-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ericof/pssa2023-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop pssa2023-frontend-acceptance
	@docker stop pssa2023-backend-acceptance
